...
object HoconEnvironmentTest extends Properties("Hocon support") with TestSupport {
  ...
  private def hoconFileIO: IO[Environment[IO]] =
    for {
      // TODO
      fp <- IO(getClass.getClassLoader.getResource("testdata.conf"))
      ep <- fromPropertiesFile[IO](fp.getFile)
      q <- IO(logging[IO](ep, silencer[IO]))
    } yield q

  private val genEnvIO: Gen[IO[Environment[IO]]] =
    Gen.const(hoconFileIO) // Gen.oneOf(Gen.const(envIO), Gen.const(hoconFileIO))
