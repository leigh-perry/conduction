        ...
        for {
          ...
          _.fold(
            e =>
              e.shouldBeNec(
                NonEmptyChain(
                  ConfiguredError.MissingValue(concat("CHOICE", "C1", "HOST")),
                  ConfiguredError.MissingValue(concat("CHOICE", "C1", "PORT")),
                  ConfiguredError.MissingValue(concat("CHOICE", "C2", "C1", "HOST")),
                  ConfiguredError.MissingValue(concat("CHOICE", "C2", "C1", "PORT")),
                  ConfiguredError.MissingValue(concat("CHOICE", "C2", "C2", "HOST")),
                  ConfiguredError.MissingValue(concat("CHOICE", "C2", "C2", "PORT"))
                )
              ),
            _ => false
          )
  ...
  property("Missing Configured[IO, List[Int]]") = forAllIO(genEnvIO(Map.empty, "empty")) {
    e =>
      for {
        env <- e
        c <- Configured[IO, List[Int]]("INTLIST").run(env)
      } yield c.shouldBe(ConfiguredError.MissingValue(concat("INTLIST", "COUNT")).invalidNec)
  }
  ...
  property("Missing Configured[IO, List[Endpoint]]") = forAllIO(genEnvIO(Map.empty, "empty")) {
    e =>
      for {
        env <- e
        c <- Configured[IO, List[Endpoint]]("EPLIST").run(env)
      } yield c.shouldBe(ConfiguredError.MissingValue(concat("EPLIST", "COUNT")).invalidNec)
  }
  ...
  property("Missing Configured[IO, List[TwoEndpoints]]") = forAllIO(genEnvIO(Map.empty, "empty")) {
    e =>
      for {
        env <- e
        c <- Configured[IO, List[TwoEndpoints]]("TEPLIST").run(env)
      } yield c.shouldBe(ConfiguredError.MissingValue(concat("TEPLIST", "COUNT")).invalidNec)
  }
