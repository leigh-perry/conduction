...
trait Configured[F[_], A] {
  ...
  def value(name: Environment.Key): Kleisli[F, Environment[F], ValidatedNec[ConfiguredError, A]]

  ////

  import Environment.concat
  ...
  def withSuffix(suffix: String): Configured[F, A] =
    new Configured[F, A] {
      override def value(
        ...
        self.value(name = concat(name, suffix))
    }
  ...
  def or[B](cb: Configured[F, B])(implicit F: Monad[F]): Configured[F, Either[A, B]] =
    new Configured[F, Either[A, B]] {
      override def value(
        ...
        self.value(concat(name, "C1")).flatMap {
          _.fold(
            errors1 =>
              cb.value(concat(name, "C2")).map {
                _.fold(
  ...
  implicit def configuredList[F[_], A](
    ...
    new Configured[F, List[A]] {
      override def value(
        ...
        Configured[F, Int](concat(name, "COUNT")).flatMap {
          _.fold(
            c => Kleisli(_ => c.invalid[List[A]].pure[F]),
            n =>
              List
                .tabulate(n)(identity)
                .traverse(i => Configured[F, A](concat(name, i.toString)))
                .map {
